  ...
  const getChartData = useCallback(async (type: 'evolution' | 'distribution' | 'billing-periods', selectedOrg?: string, selectedCycleFilter?: string) => {
    ...
    try {
      ...
      const { data: configs, error: configError } = await supabase
        ...
        // Apply same filter as Distribution chart to all charts for consistency
        ...
        if (type === 'billing-periods') {
          ...
          // For distribution, use the current cycle data from the base query (already filtered)
          // Get the most recent cycle from consumption data
