...
export function ProjectForecast() {
  ...
  const getValuesForSelectedItems = (data: any[]) => {
    ...
    const allProjectsSelected = selectedProjects.length === availableProjects.length - 1 && !selectedProjects.includes('all');

    if (selectedProjects.includes('all') || allProjectsSelected) {
        ...
        return data.map(item => {
            ...
            for (const projectId of selectedProjects) {
                const projectKey = projectId.replace(/[^a-zA-Z0-9]/g, '_');
                const dataKey = `${projectKey}_${metricType}`;
                sum += item[dataKey] || 0;
            }
            return sum;
        });
