-- Create function to get cost evolution data with virtual tag filtering
CREATE OR REPLACE FUNCTION get_cost_evolution_data_with_virtual_tag(
...
-- Create function to get organization details data with virtual tag filtering  
CREATE OR REPLACE FUNCTION get_organization_details_data_with_virtual_tag(
  org_filter text DEFAULT NULL,
  virtual_tag_filter text DEFAULT NULL
)
RETURNS TABLE (
...
BEGIN
  ...
  SELECT 
    cs.org_id,
    COALESCE(cs.org_name, cs.org_id) as org_name,
