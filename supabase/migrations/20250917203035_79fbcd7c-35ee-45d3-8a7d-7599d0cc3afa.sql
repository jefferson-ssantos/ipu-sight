-- Fix the get_dashboard_kpis_with_virtual_tag function
CREATE OR REPLACE FUNCTION public.get_dashboard_kpis_with_virtual_tag(start_date text DEFAULT NULL::text, end_date text DEFAULT NULL::text, org_filter text DEFAULT NULL::text, virtual_tag_filter text DEFAULT NULL::text)
 RETURNS TABLE(total_cost numeric, total_ipu numeric, active_orgs bigint, avg_daily_cost numeric, total_executions bigint, avg_execution_time numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
BEGIN
  ...
  SELECT c.preco_por_ipu 
  INTO client_price_per_ipu
  FROM api_clientes c
  ...
  SELECT 
    COALESCE(SUM(cd.consumption_ipu) * client_price_per_ipu, 0) as total_cost,
    ...
    CASE 
      ...
      THEN COALESCE(SUM(cd.consumption_ipu) * client_price_per_ipu, 0) / COUNT(DISTINCT cd.billing_period_start_date)
      ELSE 0 
