...
export async function validateSupabaseJWT(authHeader: string | null) {
  if (!authHeader) {
...
export async function getUserClientId(user: any) {
...
serve(async (req) => {
  ...
  try {
    return new Response(JSON.stringify({ 
      message: 'PostgreSQL connection utility loaded',
      timestamp: new Date().toISOString()
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error in postgres-connection:', error);
