...
serve(async (req) => {
  ...
  try {
    // Esta função não espera parâmetros no corpo da requisição
    const result = await pg`
      SELECT * FROM get_available_cycles(${userClientId})
    `;
    ...
    console.error('Erro ao chamar a função get_available_cycles:', err);
    if (pg) await pg.end();
